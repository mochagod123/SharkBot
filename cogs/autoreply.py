...
class AutoReplyCog(commands.Cog):
    ...
    async def do_reply(self, message: discord.Message):
        ...
        try:
            basetext = dbfind["ReplyWord"]
            if type(basetext) == str:
                randomtext = random.choice(basetext.split(","))
                await message.reply(randomtext)
        except:
            return

    @commands.Cog.listener("on_message")
    async def on_message_autoreply(self, message):
        if message.author.bot:
            return
        await self.do_reply(message)
