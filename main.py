...
async def on_ready():
    ...
    print("Ready.")
    await bot.get_channel(1342019535062171758).send(embed=discord.Embed(title="Botが起動しました。", color=discord.Color.green()).add_field(name="導入サーバー数", value=f"{len(bot.guilds)}サーバー"))
...
@bot.check
async def command_check(ctx: commands.Context):
    if not ctx.guild:
        return True
    if ctx.author.guild_permissions.administrator:
        return True
    return await is_command_enabled(ctx.guild.id, ctx.command.name)

@bot.event
async def on_message(message):
    db = bot.async_db["Main"].BANUser
    try:
        profile = await db.find_one({"User": message.author.id}, {"_id": False})
        if profile is None:
